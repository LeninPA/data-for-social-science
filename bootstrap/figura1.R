...
### Bootstrap CI for main specification ###
      ...
      RNGkind("L'Ecuyer-CMRG") # Necesario debido al paralelismo
      set.seed(1)
      # out <- c()
      s <- system.time({
      ...
      s
        ...
        # ) %dopar% {
        #   tryCatch( { 
        #       samp <- data.frame(id=sample(cc,length(cc),replace=T))   #use tryCatch() because occasionally weird samples throw an error
        #       subdata <- inner_join(data_mex,samp)
        ...
        write_csv(df,path="inputs/bootstrap_runs/BootstrapMainModel_mex.csv")
